...
namespace Inspection.Infrastructure.Configurations.Accounting.Payment.CashReceipts
{
    public class CashReceiptConfiguration : IEntityTypeConfiguration<CashReceipt>
    {
        ...
        {
            builder.ToTable("CashReceipt", "Accounting");
            ...
            // Details
            builder.HasMany(x => x.CashReceiptLines)
                   .WithOne(x => x.CashReceipt)
                   .HasForeignKey(x => x.CashReceiptId)
                   .OnDelete(DeleteBehavior.Cascade);

            builder.HasMany(x => x.CashReceiptAdjustments)
                   .WithOne(x => x.CashReceipt)
                   .HasForeignKey(x => x.CashReceiptId)
                   .OnDelete(DeleteBehavior.Cascade);

            builder.HasMany(x => x.SalesInvoiceAllocations)
                   .WithOne(x => x.CashReceipt)
                   .HasForeignKey(x => x.CashReceiptId)
                   .OnDelete(DeleteBehavior.Cascade);
