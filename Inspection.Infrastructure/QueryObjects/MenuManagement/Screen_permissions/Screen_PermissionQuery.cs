...
namespace Inspection.Infrastructure.QueryObjects.MenuManagement.Screen_permissions
{
    ...
    internal class Screen_PermissionQuery : QueryObjectBase<Screen_permissionDtoByInclude>
    {
        public Screen_PermissionQuery(
        ...
        public override async Task<ReturnBase<IEnumerable<Screen_permissionDtoByInclude>>> Query(SqlQueryOptions queryOptions)
        {
            try
            {
                string baseTable = "Screen_permission";
            ...
            ("LEFT JOIN", "[Inspection].[InspectionMethod] InspMeth", "InspMeth", "InspMeth.Id = ICKL.InspectionMethodId"),
                ...
                var result = await _dapper.QueryList<Screen_permissionDtoByInclude>(sql);

                if (!result.Succeeded)
                    return ReturnBase<IEnumerable<Screen_permissionDtoByInclude>>.Fail(result.Errors);

                var list = base.ApplyFilters(result.Result, queryOptions);

                return ReturnBase<IEnumerable<Screen_permissionDtoByInclude>>.Success(list);
            }
            catch (Exception ex)
            {
        ...
        public override Task<ReturnBase<IEnumerable<Screen_permissionDtoByInclude>>> Query(SqlQueryOptions queryOptions, string functionParameter)
        {
            throw new NotImplementedException();
        }

        public override Task<ReturnBase<IEnumerable<Screen_permissionDtoByInclude>>> Query(SqlQueryOptions queryOptions, object[] functionParameters)
        {
            throw new NotImplementedException();
        }
    }
