...
namespace Inspection.Infrastructure.Repositories.Command.Contracting.CostCodes
{
    public class CostCodeCommandRepository : CommandRepositoryBase<CostCode>, ICostCodeCommandRepository
    {
        public CostCodeCommandRepository(
            DbContext context,
        ...
        public async Task<ReturnBase> DeleteById(long id)
        {
            ...
            if (entity is null)
            {
                ...
                {
                    ErrorCode = "404",
                    ErrorMessage = $"Cost Code with Id {id} was not found"
                };
        ...
        public async Task<bool> HasChildren(long parentId)
        {
            return await _context.Set<CostCode>()
                .AnyAsync(x => x.ParentCostCodeId == parentId);
        }
    }
}