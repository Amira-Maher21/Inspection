...
namespace Inspection.Infrastructure.Repositories.Command.Accounting.AccountSetup
{
    public class CostCenterCommandRepository : CommandRepositoryBase<CostCenter>, ICostCenterCommandRepository
    {
        public CostCenterCommandRepository(DbContext context, ITenantResolver tenantResolver, IExceptionManager exceptionManager) : base(context, tenantResolver, exceptionManager)
        {
        ...
        public async Task<bool> HasChildren(long parentId)
        {
            return await _context.Set<CostCenter>()
                .AnyAsync(x => x.ParentCostCenterId == parentId);
        }
